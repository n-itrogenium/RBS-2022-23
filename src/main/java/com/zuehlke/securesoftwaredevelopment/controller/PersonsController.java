...
import com.zuehlke.securesoftwaredevelopment.repository.MovieRepository;
...
public class PersonsController {
    ...
    public String person(@PathVariable int id, Model model, HttpSession session) {
        if(!SecurityUtil.hasPermission("VIEW_PERSON") && SecurityUtil.getCurrentUser().getId() != id) {
            AuditLogger.getAuditLogger(PersonsController.class).audit("Unauthorized user '"
                    + SecurityUtil.getCurrentUser().getUsername() + "' tried to view person page");
    ...
    public String updatePerson(Person person, HttpSession session, @RequestParam("csrfToken") String csrfToken)
        ...
        String loggedUserId = "" + SecurityUtil.getCurrentUser().getId();
        if(!SecurityUtil.hasPermission("VIEW_PERSON") && !loggedUserId.equals(person.getId())) {
            AuditLogger.getAuditLogger(PersonsController.class).audit("Unauthorized user '"
                    + SecurityUtil.getCurrentUser().getUsername() + "' tried to update person " + person.getId());
            throw new AccessDeniedException("Forbidden");
        }
        String csrf = session.getAttribute("CSRF_TOKEN").toString();
