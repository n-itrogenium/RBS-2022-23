...
public class MovieRepository {
    ...
    public List<Movie> getAll() {
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            LOG.info("Movies successfully fetched");
        } catch (SQLException e) {
            e.printStackTrace();
            LOG.warn("Movies fetch failed", e);
        }
        return movieList;
    }
    ...
    public List<Movie> search(String searchTerm) throws SQLException {
        ...
        LOG.info("Movie search successful");
        return movieList;
    }
    ...
    public Movie get(int movieId, List<Genre> genreList) {
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            while (rs.next()) {
                ...
                while (rs2.next()) {
                    Genre genre = genreList.stream().filter(g -> {
                        try {
                            ...
                            LOG.warn("Query for genre " + g.getId() + " failed", e);
                ...
                LOG.info("Movie " + movieId + " fetch successful");
                return movie;
            }
        } catch (SQLException e) {
            e.printStackTrace();
            LOG.warn("Movie " + movieId + " fetch failed", e);
        }
            ...
            if (generatedKeys.next()) {
                ...
                genresToInsert.stream().forEach(genre -> {
                    ...
                    try (PreparedStatement statement2 = connection.prepareStatement(query2);
                        ...
                        e.printStackTrace();
                        LOG.warn("Assigning genre to movie failed", e);
                    }
                });
                LOG.info("Genres successfully assigned to new movie " + id);
            }
        } catch (SQLException e) {
            e.printStackTrace();
            AuditLogger.getAuditLogger(MovieRepository.class).audit("Creating new movie " + id + " by '"
                    + SecurityUtil.getCurrentUser().getUsername() + "' failed");
        }
        AuditLogger.getAuditLogger(MovieRepository.class).audit("New movie " + id + " created by '" +
    ...
    public void delete(int movieId) {
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            e.printStackTrace();
            AuditLogger.getAuditLogger(MovieRepository.class).audit("Deleting movie " + movieId + " by '"
                    + SecurityUtil.getCurrentUser().getUsername() + "' failed");
        }
        AuditLogger.getAuditLogger(MovieRepository.class).audit("Movie " + movieId + " deleted by '"
                + SecurityUtil.getCurrentUser().getUsername());
    }
